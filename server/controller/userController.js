...
export const getSubjectDetails = async (req, res) => {
    try {
        ...
        const subDetails = await Subject.findById(subId).populate("teachers", 'name avatar about');
...
export const getVideo = async (req, res) => {
    try {
        ...
        const video = teacherVideo.level[level].find((item) => item._id.toString() === videoId.toString());
        if(!video){
            return res.status(404).json({
...
export const getAvilableComments = async (req, res) => {
    try {
        ...
        const comments = await Comment.find({ show: true }).populate('user', 'name email level phone');

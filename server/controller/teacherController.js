...
export const getTeacherById = async (req, res) => {
    try {
         ...
         const teacher = await Teacher.findById(teacherId).select('-password').populate('subjects');
...
export const teacherDeleteVideo = async (req, res) => {
    try {
        ...
        const deletedVideo = await VideoModel.findOneAndDelete({_id: videoId, teacher: teacherId});
        if(!deletedVideo){
            return res.status(404).json({
...
export const teacherUpdateVideo = async (req, res) => {
    try {
        ...
        const isVideoExist = await VideoModel.findOne({_id: videoId, teacher: teacherId});
        if(!isVideoExist){
            return res.status(404).json({
...
export const deleteQuize = async (req, res) => {
    try {
        ...
        const deletedQuiz = await Quizz.findOneAndDelete({_id: quizId, teacher: teacherId});
...
export const deletePdf = async (req, res) => {
    try {
            ...
            const deletedPdf = await PdfModel.findOneAndDelete({teacher: teacherId, _id: pdfId});
            if(!deletedPdf){
                return res.status(404).json({
                    message: "Pdf not found",
                    error: true,
...
export const teacherStatics = async (req, res) => {
    try {
        ...
        const firstLevelVideosLength = teacherVideos.filter(video => video.level === "first").length;
        const secondLevelVideosLength = teacherVideos.filter(video => video.level === "second").length;
        const thirdLevelVideosLength = teacherVideos.filter(video => video.level === "third").length;

        const teacherQuizzes = await Quizz.find({teacher: teacherId});

        const teacherQuizzesLength = teacherQuizzes.length;
        const firstLevelQuizzesLength = teacherQuizzes.filter(quiz => quiz.level === "first").length;
        const secondLevelQuizzesLength = teacherQuizzes.filter(quiz => quiz.level === "second").length;
        const thirdLevelQuizzesLength = teacherQuizzes.filter(quiz => quiz.level === "third").length;

        const teacherNotes = await PdfModel.find({teacher: teacherId});

        const teacherNotesLength = teacherNotes.length;
        const firstLevelNotesLength = teacherNotes.filter(note => note.level === "first").length;
        const secondLevelNotesLength = teacherNotes.filter(note => note.level === "second").length;
        const thirdLevelNotesLength = teacherNotes.filter(note => note.level === "third").length;

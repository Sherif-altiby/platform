...
import { RegisterInputs, registerSchema } from "@/validations/registerValidation";
...
const page = () => {
  ...
  const { register, handleSubmit, formState: { errors }, reset} = useForm<RegisterInputs>({
    mode: "onBlur",
    resolver: zodResolver(registerSchema)
  })

  const onSubmit: SubmitHandler<RegisterInputs> = async (data) =>{
    await userRegister(data.name, data.email, data.password, data.level, data.phone);
    ...
    <div className="flex items-center justify-center defualt-height" >
         <form className="w-[90%] shadow-lg max-w-[700px] mt-10 mb-10 p-5 rounded-lg" onSubmit={handleSubmit(onSubmit)} >
             ...
             <div className="flex items-start flex-col md:flex-row gap-5 mb-5" >
                  <div className="w-full md:w-1/2" >
                      <label className="block text-grayColor text-lg mb-2" htmlFor="name"> الاسم ثلاثي</label>
                      <input 
                        type="text" 
                        id="name" 
                        className={`border rounded-md p-2 block w-full transition-all duration-300 focus:border-hoverLinkColor ${errors.name && 'border-red-700'} `} 
                        {...register("name")}
                        />
             ...
             <div className="flex items-start flex-col md:flex-row gap-5 mb-5" >
                  <div className="w-full md:w-1/2" >
                      ...
                      <input 
                        type="number" 
                        id="phone" 
                        className={`border rounded-md p-2 block w-full transition-all duration-300 focus:border-hoverLinkColor ${errors.phone && 'border-red-700'} `} 
                        {...register("phone")}
                        />
                  ...
                  <div className="w-full md:w-1/2" >
                    <label className="block text-grayColor text-lg mb-2" htmlFor="level"> الصف الدراسي </label>
                     <select 
                       id="level" 
                       className={`border rounded-md p-2 block w-full transition-all duration-300 focus:border-hoverLinkColor ${errors.level && 'border-red-700'} `}
                       {...register("level")}
                       >
             ...
             <div className="flex items-start flex-col md:flex-row gap-5 mb-5" >
                  ...
                  <div className="w-full md:w-1/2" >
                    <label className="block text-grayColor text-lg mb-2" htmlFor="pass-confirm">   تاكيد كلمة المرور </label>
                    <input 
                       type="password" 
                       id="pass-confirm" 
                       className={`border rounded-md p-2 block w-full transition-all duration-300 focus:border-hoverLinkColor ${errors.confirmPassword && 'border-red-700'} `} 
         ...
         <button className="flex items-center justify-center md:text-lg   h-[50px] rounded-xl w-full sm:w-[170px] bg-hoverLinkColor border border-hoverLinkColor text-white transition-all duration-500 hover:rounded-[50px] hover:bg-white hover:text-hoverLinkColor" >
             ...
             <div className="flex items-center justify-center gap-1 mt-5 text-lg" >
                  ...
                  <Link href={"/login"} className="text-hoverLinkColor" > تسجيل الدخول </Link>
             </div>

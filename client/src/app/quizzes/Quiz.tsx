        ...
        <div>
          ...
          {quiz?.lessons && (
            <div className="flex flex-wrap gap-2 mt-2">
              {quiz.lessons.map((l: any) => (
                <span
                  key={l._id}
                  className="max-w-[160px] px-2 py-1 text-[11px] text-gray-700 bg-gray-100 border border-gray-200 rounded-full flex items-center"
                  title={l.title}
                >
                  <span className="truncate block w-full">{l.title}</span>
                </span>
              ))}
            </div>
          )}

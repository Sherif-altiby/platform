...
const Aboutus = () => {
  ...
  const svgRef = useRef(null); // قمنا بتغيير imageRef إلى svgRef

  useEffect(() => {
    const ctx = gsap.context(() => {
      // انيميشن لحاوية الـ SVG بالكامل
      ...
      // انيميشن للعناصر الداخلية في الـ SVG (Stagger effect)
      // سنقوم بتحريك الأشكال الهندسية داخل الـ SVG بتتابع
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-[#F8FAFC] relative overflow-hidden text-slate-900">
      
      {/* عناصر خلفية ناعمة */}
      <div className="absolute -top-24 -left-24 w-96 h-96 bg-blue-100/50 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute top-1/2 right-0 w-64 h-64 bg-orange-100/40 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row-reverse items-center gap-16">
          ...
          <div ref={textRef} className="w-full md:w-1/2 space-y-8 text-right">
            ...
            <div className="about-text-item space-y-6">
              <p className="text-slate-600 text-lg leading-relaxed">
                في منصة العبقري، نؤمن أن التعليم لا يجب أن يكون مملاً. لقد صممنا بيئة تجمع بين <span className="font-bold text-blue-600">متعة التعلم</span> وقوة الأدوات الرقمية، لنمنحك القدرة على التفوق في دراستك بأسلوب لم تعهده من قبل.
              </p>
            </div>

            {/* المميزات بتصميم "Cards" خفيف */}
            <div className="about-text-item grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
              {[
                ...
                <div key={i} className="flex items-center justify-end gap-3 p-3 bg-white rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow group">
                  <span className="text-slate-700 font-bold group-hover:text-blue-600 transition-colors">{item.title}</span>
          ...
          {/* الجانب البصري: مشهد SVG تفاعلي بدلاً من الصورة */}
              ...
              <div className="relative z-10 rounded-[2rem] p-6 bg-white shadow-2xl shadow-blue-900/10 drop-shadow-lg">
                ...
                >
                  ...
                  <g className="animate-float">
                    ...
                    {/* الكتاب/التابلت */}
                    <rect x="160" y="200" width="140" height="90" rx="10" fill="white" stroke="#3B82F6" strokeWidth="3" shadow-lg="true" className="svg-element" />
                  ...
                  {/* عناصر عائمة ملونة (Abstract shapes) */}
                ...
                {/* إحصائية سريعة "Floating" كما هي */}
                <div className="absolute top-6 left-6 bg-white/90 backdrop-blur-md px-6 py-4 rounded-2xl shadow-lg border border-white flex flex-col items-center animate-float">

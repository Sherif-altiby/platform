...
import { Autoplay, Pagination } from "swiper/modules"; // ✅ Fix 4a: import Pagination
import "swiper/css";
...
// ✅ Fix 1: module-level constant — allocated once, never on re-render
const TESTIMONIALS = [
  {
...
// ✅ Fix 4b: stable references — Swiper won't re-init its config on every render
const SWIPER_MODULES = [Autoplay, Pagination];
const SWIPER_AUTOPLAY = { delay: 4000, disableOnInteraction: false };
...
const Tesmonials = () => {
  ...
  const headingRef = useRef<HTMLDivElement>(null);   // ✅ Fix 2b: real refs for triggers
  const swiperRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      // ✅ Fix 2b: pass DOM refs directly — no global class-name query
    ...
    // ✅ Fix 2a: recalculate trigger positions after layout is painted
  ...
  return (
    ...
    >
      {/* ✅ Fix 3: will-change + translateZ to isolate expensive blur layers */}
      <div
        className="absolute top-0 right-[-10%] w-[500px] h-[500px] bg-blue-50/50 rounded-full blur-[120px] pointer-events-none"
        style={{ willChange: "filter", transform: "translateZ(0)" }}
      />
      <div
        className="absolute bottom-[-10%] left-[-5%] w-[400px] h-[400px] bg-orange-50/50 rounded-full blur-[100px] pointer-events-none"
        style={{ willChange: "filter", transform: "translateZ(0)" }}
      />

      <div className="container mx-auto px-6 relative z-10">
        <div ref={headingRef} className="mb-20 text-center relative"> {/* ✅ ref replaces class trigger */}
          ...
          <p className="text-slate-500 text-lg md:text-xl max-w-2xl mx-auto leading-relaxed font-medium">
            نحن نفخر بكوننا جزءاً من رحلة نجاح{" "}
            <span className="text-slate-900 font-bold">آلاف الطلاب</span>، إليك
            بعض قصصهم الملهمة التي تعكس شغفنا بالتميز.
          </p>
        ...
        <div ref={swiperRef} className="px-4"> {/* ✅ ref replaces class trigger */}
          <Swiper
